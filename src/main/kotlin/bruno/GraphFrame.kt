...
import com.sun.java.accessibility.util.SwingEventMonitor.addAncestorListener
...
class Graphable(
        val f: (Double) -> Double,
        val name: String = "",
        val rangep: Pair<Double, Double>? = null,
        val pois:Map<Point,String>?=emptyMap(), // why is ? required
...
class GraphFrame(title: String) : JFrame() {
    ...
    val graphPanel = object : JPanel() {
        ...
        fun paintGraphable(g: Graphics, fi: Int) {
            var graphable = graphables[fi]


            if (graphable.pois != null) {
                for (point in graphable.pois!!.keys) {
                    val label = graphable.pois!![point]
                    val xp = xPix2Value.v2pix(point.x)
                    val yp = yPix2Value.v2pix(point.y)
                    g.drawRect(xp - 3, yp - 3, 6, 6)
                    g.drawLine(xp + 6, yp + 6, xp + 20, yp + 20)
                    g.drawString(label, xp + 20, yp + 40)
                }
            }
            ...
            g.drawString(graphable.name + (graphable.description ?: ""), 200, fi * 50 + 20)
...
//	}
        ...
        fun recomputeBoundaries() {
            ...
            xPix2Value = Pix2Value(viewPortWidth, false, min.x, max.x)
            yPix2Value = Pix2Value(viewPortHeight, true, min.y, max.y)
        ...
        fun setXRange(range: Pair<Double, Double>) {
            centerp = center.x((range.first + range.second) / 2.0)
            center = centerp
            area = area.x(abs(range.second - range.first))
            recomputeBoundaries()
        }
            ...
            graphPanel.addComponentListener(
                    object : ComponentAdapter() {
                        override fun componentResized(e: ComponentEvent) {
                            viewPortWidth = graphPanel.width.toInt()
                            viewPortHeight = graphPanel.height.toInt()
                            expandArea2Resized()
                        }
                    }
            );
                        ...
                        object : MouseMotionAdapter() {
                            ...
                            override fun mouseDragged(e: MouseEvent) {
                                ...
                                if (dragging == false && dragStarted != null)
                                    dragStarted.accept(lastPressMouseEvent!!)
                                dragging = true

                                // the real way
                                if (dragMove != null)
                                    dragMove.accept(lastPressMouseEvent!!, e)
                            }
